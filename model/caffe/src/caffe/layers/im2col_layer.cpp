...
void Im2colLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  jump_period_.Reshape(dim_blob_shape);
  jump_step_.Reshape(dim_blob_shape);
  int* jump_period_data = jump_period_.mutable_cpu_data();
  int* jump_step_data = jump_step_.mutable_cpu_data();
  if (conv_param.has_jump_period()) {
    jump_period_data[0] = conv_param.jump_period();
    jump_step_data[0] = conv_param.jump_step();
  }
  else {
    jump_period_data[0] = 1;
    jump_step_data[0] = 0;
  }
...
void Im2colLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  const int* jump_period_data = jump_period_.cpu_data();
  const int* jump_step_data = jump_step_.cpu_data();
  for (int i = 0; i < num_spatial_axes_; ++i) {
    ...
    const int output_dim = (input_dim + 2 * pad_data[i] - kernel_extent)
        / stride_data[i] + 1;*/
    const int output_dim = jump_step_data[0] == 0?
        (input_dim + 2 * pad_data[i] - kernel_shape_data[i] * dilation_data[i])
        / stride_data[i] + dilation_data[i] :
        input_dim * jump_period_data[0] / (jump_period_data[0]+jump_step_data[0]);
  ...
  for (int n = 0; n < num_; ++n) {
    ...
    if (!force_nd_im2col_ && num_spatial_axes_ == 2) {
      im2col_cpu(bottom_data + n * bottom_dim_, channels_,
          ...
          jump_period_.cpu_data()[0], jump_step_.cpu_data()[0],
          top_data + n * top_dim_);
    } else {
