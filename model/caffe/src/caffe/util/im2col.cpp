...
void im2col_cpu(const Dtype* data_im, const int channels,
  ...
  const int height_col = jump_step == 0? (height + 2 * pad_h - kernel_h * dilation_h) / stride_h + dilation_h :
      height * jump_period / (jump_period + jump_step); // output height
  const int width_col = jump_step == 0? (width + 2 * pad_w - kernel_w * dilation_w) / stride_w + dilation_w :
      width * jump_period / (jump_period + jump_step); // output width
  const int channels_col = channels * kernel_h * kernel_w;  // output channel
  for (int c_col = 0; c_col < channels_col; ++c_col) {
    int w_offset = (c_col % kernel_w) * dilation_w;
    w_offset += (w_offset / jump_period) * jump_step;
    int h_offset = ((c_col / kernel_w) % kernel_h) * dilation_h;
